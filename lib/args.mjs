...
export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === 'dashboard') out.dashboard = true;
    else if (a === '--interval') out.interval = Number(argv[++i]) || null;
    else if (a === '--once') out.once = true;
    else if (a === '--plain') out.plain = true;
    else if (a === '--no-usage') out.noUsage = true;
