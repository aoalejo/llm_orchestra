...
const DEFAULT_CACHE_MS = 10 * 60 * 1000;   // 10 min; override con keys.usageCacheMs
...
export async function fetchUsage(config, keyValue, { timeoutMs = 8000, noCache = false } = {}) {
  ...
  if (!noCache && hit && Date.now() - hit.ts < (config?.keys?.usageCacheMs ?? DEFAULT_CACHE_MS)) return hit.data;
...
/** Mapa `valorDeKey -> usage` para que `pickKey` elija la cuenta con más cuota. */
export async function keyQuotaMap(config, { noCache = false } = {}) {
  const map = {};
  const orchVal = config?.keys?.orchestrator ? process.env[config.keys.orchestrator] : null;
  if (orchVal) map[orchVal] = await fetchUsage(config, orchVal, { noCache });
  for (const k of workerKeyEntries(config)) map[k.value] = await fetchUsage(config, k.value, { noCache });
  return map;
}
