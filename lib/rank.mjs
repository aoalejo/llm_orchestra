...
/** Roles de servicio que `--apply` puede actualizar con el modelo recomendado. */
export const SERVICE_ROLES = ['scout', 'scribe', 'security', 'merge'];
 ...
 *   exclude             [modelId, ...] que nunca se usan (blocklist)
 *   pins                {author, verifier, escalationAuthor, escalationVerifier,
 *                        service, scout, scribe, security, merge} fuerzan el rol
 */
export function rankModels(catalog, rows, opts = {}) {
  ...
  const excluded = new Set((opts.exclude || []).map(String));
  ...
  // Pins: fuerzan el rol sin importar el ranking (por si el usuario quiere
  // congelar un modelo aunque salga algo mejor/barato).
  const asPin = (v) => (Array.isArray(v) ? v.filter(Boolean).map(String) : v ? [String(v)] : null);
  const pinAuthor = asPin(pins.author);
  const pinVerifier = asPin(pins.verifier);
  let author = pinAuthor?.length ? [...pinAuthor] : pool.slice(0, workersPerRole).map((a) => a.id);
  // El verifier rota la lista para que, con los mismos índices, nunca coincida con el autor.
  let verifier = pinVerifier?.length ? [...pinVerifier] : (author.length > 1 ? [...author.slice(1), author[0]] : [...author]);

  let escalationAuthor = pins.escalationAuthor ? String(pins.escalationAuthor) : (matched[0]?.id ?? pool[0]?.id ?? null);
  let escalationVerifier = pins.escalationVerifier ? String(pins.escalationVerifier)
    : ((matched.find((a) => a.id !== escalationAuthor) || pool.find((a) => a.id !== escalationAuthor))?.id ?? null);
  ...
  // Modelo para roles de servicio (scout/scribe/security/merge): el mejor barato del pool.
  const service = pins.service ? String(pins.service) : (author[0] ?? escalationAuthor ?? null);
  const serviceRoles = {};
  for (const role of SERVICE_ROLES) serviceRoles[role] = pins[role] ? String(pins[role]) : service;
  return {
    counts: {
    ...
    service,
    serviceRoles,
    pins: Object.keys(pins).filter((k) => {
...
export function configPatchFromRanking(r, opts = {}) {
  ...
  if (opts.serviceRoles !== false && (r.serviceRoles || r.service)) {
    for (const role of SERVICE_ROLES) patch.roles[role] = r.serviceRoles?.[role] || r.service;
  }
