...
export function rankModels(catalog, rows, opts = {}) {
  ...
  const service = author[0] ?? escalationAuthor ?? null;
  return {
    counts: {
...
/** Roles de servicio que `--apply` actualiza con el modelo recomendado. */
...
export function configPatchFromRanking(r, opts = {}) {
  ...
  if (opts.serviceRoles !== false && r.service) {
    for (const role of SERVICE_ROLES) patch.roles[role] = r.service;
  }
