...
export function statusCommand(args, config) {
  if (args.run || args.runs) {
    ...
    const o = { ts: now(), runId, status: marker?.status ?? 'not-found', startedAt: marker?.startedAt ?? null, pid: marker?.pid ?? null, tasks, log: logFile, tail };
    emit(args, o, () => { log(`run ${runId}: ${o.status} (pid ${o.pid})`); for (const t of tasks) log(`  ${t.id}: ${t.state} $${t.cost}`); if (tail) log(tail); });
    return o;
  }
  const doc = readTasks();
