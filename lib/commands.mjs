...
import { callModel, changedFiles, runProcess } from './runner.mjs';
import { prepareWorktree, removeWorktree } from './worktrees.mjs';
import { runTaskLoop } from './loop.mjs';
import { integrateTask } from './integrate.mjs';
import { socraticodeGather, socraticodeOptions, scoutCacheKey, gitHead, readScoutCache, writeScoutCache, buildScoutPrompt, looksLikeMap } from './scout.mjs';
import { usageSnapshot } from './usage.mjs';
import { extractPaths } from './guards.mjs';
...
export async function executeTask(deps, task, opts = {}) {
  ...
  const wt = await deps.queues.git(() => prepareWorktree(config, task, { reuse }));
  ...
  if (wt.ephemeral) {
    if (integrated) await deps.queues.git(() => removeWorktree(config, task));
...
export async function scoutCommand(args, config, deps) {
  ...
  let map = (out.text || '').trim();
  const source = external?.ok ? 'socraticode+llm' : 'llm';
  // T-10: si la salida no parece un mapa, reintentar una vez con formato explícito.
  if (!out.timedOut && config.scout?.validate !== false && !looksLikeMap(map)) {
    warn('scout: la salida no tiene formato de mapa; reintento con formato explícito');
    const retry = await callModel({
      runner: deps.runner, pi: deps.pi, provider: config.provider, model: config.scout?.retryModel || config.roles.scout, apiKey: key2.value,
      systemPrompt: readAgent('scout'),
      prompt: buildScoutPrompt({ query, scope, task, external }) + '\n\nFORMATO OBLIGATORIO: una línea por hallazgo "path:line — descripción". Máximo 40 líneas. Sin prosa, sin código.',
      tools: ['read', 'grep', 'find', 'ls', 'bash'], logFile: path.join(dir, 'scout.retry.json'), cwd: deps.workdir, role: 'scout',
    });
    if (!retry.timedOut && looksLikeMap((retry.text || '').trim())) { out = retry; map = retry.text.trim(); }
  }
  const valid = looksLikeMap(map);
  const result = {
    status: out.timedOut ? 'timeout' : (valid ? 'ok' : 'invalid'),
    source,
    map,
  ...
  if (result.status === 'invalid') warn('scout: salida inválida (no se cachea); revisá el prompt o el modelo');
...
function driverPath() { return path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'orchestra.mjs'); }

/** T-04: lanza el dispatch en un proceso detached y devuelve el runId de inmediato. */
function startDetachedDispatch(args, orders, config) {
  const runId = args.runId || ('batch-' + Date.now().toString(36));
  ensureDir(RUNS);
  const logFile = path.join(RUNS, runId + '.log');
  const workers = args.workers || config.loop?.maxParallelTasks || 4;
  const argv = ['dispatch'];
  for (const o of orders) argv.push('--order', JSON.stringify(o));
  argv.push('--workers', String(workers));
  if (args.commit) argv.push('--commit');
  if (args.yes) argv.push('--yes');
  if (args.decisionsRaw) argv.push('--decisions', args.decisionsRaw);
  if (args.stub || process.env.ORCHESTRA_RUNNER === 'stub') argv.push('--stub');
  argv.push('--run-id', runId);
  const fd = fs.openSync(logFile, 'a');
  const child = spawn(process.execPath, [driverPath(), ...argv], { cwd: ROOT, detached: true, stdio: ['ignore', fd, fd], windowsHide: true });
  child.unref();
  writeJson(path.join(RUNS, runId + '.json'), { runId, pid: child.pid, startedAt: now(), status: 'running', orders: orders.map((o) => o.id), logFile });
  return { status: 'detached', runId, pid: child.pid, log: logFile, orders: orders.map((o) => o.id) };
}
...
export async function dispatchCommand(args, config, deps) {
  ...
  for (const r of raw) {
    ...
    // T-08 / T-05(b): lint de acceptance (protegidas / gitignored / inexistentes).
    if (config.guards?.lintAcceptance !== false) {
      for (const p of extractPaths([...(wo.acceptance || []), wo.goal].join('\n'))) {
        if (isProtected(config, { scope: [p] })) warn(`acceptance ${wo.id}: ${p} toca una ruta protegida`);
        else if (!exists(path.join(ROOT, p))) warn(`acceptance ${wo.id}: ${p} no existe en el proyecto (¿fixture faltante?)`);
        else {
          const gi = await runProcess('git', ['check-ignore', '--', p], { cwd: ROOT });
          if (gi.code === 0) warn(`acceptance ${wo.id}: ${p} está ignorado por git (dato local/confidencial?)`);
        }
      }
    }
  ...
  // T-04: modo detached — devuelve el runId ya y sigue en background.
  if (args.detach) {
    const detached = startDetachedDispatch(args, orders, config);
    emit(args, detached, () => log(`dispatch detached: ${detached.runId} (pid ${detached.pid}) → ${detached.log}`));
    return detached;
  }

  const runMarker = args.runId ? path.join(RUNS, args.runId + '.json') : null;
  if (runMarker) writeJson(runMarker, { runId: args.runId, pid: process.pid, startedAt: now(), status: 'running', orders: orders.map((o) => o.id), logFile: path.join(RUNS, args.runId + '.log') });
  ...
  if (runMarker) writeJson(runMarker, { runId: args.runId, pid: process.pid, startedAt: safeRead(runMarker)?.startedAt || now(), status: 'done', orders: orders.map((o) => o.id), cost: out.cost, results: results.map((r) => ({ id: r.id, status: r.status, approved: !!r.approved })) });
  emit(args, out, () => {
...
export function statusCommand(args, config) {
  if (args.run || args.runs) {
    if (args.runs || !args.run) {
      const runs = safeReaddir(RUNS).filter((e) => e.isFile() && e.name.endsWith('.json')).map((e) => safeRead(path.join(RUNS, e.name))).filter((m) => m && m.runId)
        .map((m) => ({ runId: m.runId, status: m.status, startedAt: m.startedAt, orders: m.orders }));
      const o = { ts: now(), runs };
      emit(args, o, () => { if (!runs.length) log('sin runs'); for (const r of runs) log(`${r.runId}  ${r.status}  ${(r.orders || []).join(', ')}`); });
      return o;
    }
    const runId = String(args.run);
    const marker = safeRead(path.join(RUNS, runId + '.json'));
    const pid = marker?.pid ?? null;
    let alive = null;
    if (pid) { try { process.kill(pid, 0); alive = true; } catch { alive = false; } }
    const rawStatus = marker?.status ?? 'not-found';
    const status = rawStatus === 'running' && alive === false ? 'dead' : rawStatus;
    const logFile = path.join(RUNS, runId + '.log');
    let tail = '';
    try { const lines = fs.readFileSync(logFile, 'utf8').trim().split(/\r?\n/); tail = lines.slice(-15).join('\n'); } catch { /* sin log */ }
    const tasks = (marker?.orders || []).map((id) => { const s = safeRead(STATE_FILE(id)); return { id, state: s?.status ?? null, cycle: s?.cycle ?? 0, cost: Number(((s?.spentUsd) || 0).toFixed(6)), decision: s?.decision ?? null }; });
    const o = { ts: now(), runId, status, pidAlive: alive, startedAt: marker?.startedAt ?? null, pid, tasks, log: logFile, tail };
    emit(args, o, () => { log(`run ${runId}: ${o.status}${alive === false ? ' (proceso muerto)' : ''} (pid ${o.pid})`); for (const t of tasks) log(`  ${t.id}: ${t.state} $${t.cost}`); if (tail) log(tail); });
    return o;
  }
  const doc = readTasks();
