...
import { O, RUNS, ROOT } from './paths.mjs';
import { ensureDir, exists, readJson, writeJson, now, makeQueue, runWithConcurrency } from './util.mjs';
...
export async function scoutCommand(args, config, deps) {
  ...
  ensureDir(dir);
  const key = deps.runner === 'stub' ? { name: 'stub', value: 'stub' } : pickKey(config, deps.keyState, 'worker');
  if (!key) {
  ...
  const out = await callModel({
    runner: deps.runner, pi: deps.pi, provider: config.provider, model: config.roles.scout, apiKey: key.value,
    systemPrompt: readAgent('scout'), prompt,
  ...
  writeJson(path.join(dir, 'scout.map.json'), { ts: now(), query, scope, ...result });
