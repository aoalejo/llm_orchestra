...
/**
 * ¿La llamada al modelo se cayó por TRANSPORTE (no hubo veredicto del rol)? El proveedor corta
 * con `400 event: error` cuando el contexto se infla — visto en verifiers que volcaban `gate.log`
 * entero con `cat` varias veces. Es reintentable: NO es un veredicto.
 */
export function isTransportError(out) {
  return !!out && out.timedOut !== true && (out.stopReason === 'error' || !!out.errorMessage);
}

/** ¿La verificación quedó sin veredicto utilizable? (timeout o error de transporte) */
export function noUsableVerdict(out) {
  return !!out && (out.timedOut === true || isTransportError(out));
}

/**
 * Política del baseline de gates (T-03b). El gate exige verde TOTAL en el worktree del autor:
 * el baseline NO descuenta fallos preexistentes. Si los gates ya fallan en el worktree limpio,
 * ningún ciclo puede pasar, así el loop corta en el ciclo 0 en vez de quemar ciclos (y plata)
 * contra fallos que el autor no introdujo (medido: 4 ciclos y $0.118 peleando un baseline rojo).
 * Overrides: `decisions.gate === 'continue'` (lo manda el orquestador al reintentar) o
 * `gates.allowRedBaseline: true` (config del proyecto).
 */
export function baselinePolicy({ ok, decisions, gates } = {}) {
  if (ok !== false) return 'continue'; // pasó, o el baseline está desactivado
  if (decisions?.gate === 'continue') return 'continue';
  if (gates?.allowRedBaseline === true) return 'continue';
  return 'abort';
}
...
export function pickAuthorVerifier(config, cycle, maxCycles, forced = null) {
  ...
  // ADR 0005 (Paisanitos): el escalado a los modelos caros NO es automático.
  // Sólo se usa el par de escalado si el orquestador lo habilita explícitamente
  // (`loop.autoEscalate: true`) o si fuerza el modelo (`forced`). En una corrida
  // real el escalado automático se llevó 1,06 de 1,45 USD sin aportar veredicto.
  const last = cycle === maxCycles && config.loop?.autoEscalate === true;
