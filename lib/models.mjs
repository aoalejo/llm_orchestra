...
/** Key del provider: env de A, alguna de B (array), o `auth.json` de pi. Nunca se loguea. */
export function resolveProviderKey(config) {
  const orch = config?.keys?.orchestrator ? process.env[config.keys.orchestrator] : null;
  if (orch) return orch;
  const worker = workerKeyEntries(config)[0];
...
export function rankingsStale(generatedAt, maxAgeHours = 24) {
  if (!generatedAt) return true;
  return Date.now() - new Date(generatedAt).getTime() > maxAgeHours * 3600e3;
}

/** Horas que pueden pasar antes de refrescar (acepta maxAgeHours o maxAgeDays legacy). */
export function maxAgeHoursOf(config) {
  const r = config?.models?.rankings || {};
  if (Number.isFinite(r.maxAgeHours)) return r.maxAgeHours;
  if (Number.isFinite(r.maxAgeDays)) return r.maxAgeDays * 24;
  return 24;
}
