...
`node orchestra.mjs --self-test` valida lógica pura sin red (hoy **80 casos**):
...
`runProcess` (timeout), `normalizeWorkOrder`, `validateWorkOrder`, `compactFindings`, `slugify`.
...
y verifica **31 invariantes**: dry-run no muta el backlog, ruta protegida sin/con `--yes`,
